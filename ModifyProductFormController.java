...
public class ModifyProductFormController implements Initializable {
    ...
    void onActionSave(ActionEvent event) throws IOException {
        ...
        Alert confirmationAlert = new Alert(Alert.AlertType.CONFIRMATION, "This will permanently save the modifications to this part.\n" +
                "Do you want to continue?");
        ...
        if (confirmationResult.isPresent() && confirmationResult.get() == ButtonType.OK) {
            try {
                ...
                if (MIN > MAX || INV>MAX || INV<MIN) {
                    ...
                    int ctr = 0;
                    for (Product productInInventory : Inventory.getAllProduct()) {
                        if (ID == productInInventory.getId()) {
                            break;
                        }
                        ctr++;
                    }

                    Inventory.getAllProduct().get(ctr).setName(NAME);
                    Inventory.getAllProduct().get(ctr).setStock(INV);
                    Inventory.getAllProduct().get(ctr).setPrice(PRICE);
                    Inventory.getAllProduct().get(ctr).setMin(MIN);
                    Inventory.getAllProduct().get(ctr).setMax(MAX);

                    //add the new parts
                    for (Part listedPart : assocParts) {
                        if (Inventory.getAllProduct().get(ctr).getAllAssociatedParts().contains(listedPart) == false) {
                            Inventory.getAllProduct().get(ctr).addAssociatedPart(listedPart);
                        }
                    }

                    //remove parts taken off of list
                    for (Part listedPart : Inventory.getAllProduct().get(ctr).getAllAssociatedParts()) {
                        if (assocParts.contains(listedPart) == false) {
                            Inventory.getAllProduct().get(ctr).deleteAssociatedPart(listedPart);
                        }
                    }
    ...
    /**
     * Sends product information from MainMenu to ModifyProductForm
     * @param selectedProduct product selected in the MainMenu table
     */
    public void sendAssocParts(Product selectedProduct){
    ...
    /**
     * intitializes ModifyProductForm.fxml and populates the tables
     * @param url
     * @param rb
     */
    @Override
